...
def read_invoice_data(file_path):
    ...
    try:
        ...
        for i in range(data_start_row, len(invoice_df)):
            ...
            # 若品名为空，继承上一行的品名（处理合并单元格）
            if not first_col_val or first_col_val.lower() == "nan":
                first_col_val = last_valid_name
            else:
                last_valid_name = first_col_val
            ...
            try:
                if isinstance(item[4], (int, float)) and item[4] > 0:
                    ...
                    qty_str = str(item[4]).replace(",", "").strip()
                    match = re.search(r"[-+]?\d*\.?\d+", qty_str)
                    if match:
                        qty_float = float(match.group())
                        if qty_float > 0:
                            item[4] = qty_float
                            qty_valid = True
            except Exception:
...
def create_export_declaration(invoice_data, template_path, output_dir, user_inputs):
    ...
    try:
        ...
        factory = user_inputs.get("factory", "东莞致嘉")
        ...
        if factory == "惠州致嘉":
            # 先替换完整的境内发货人信息
            replacements["441996482N 东莞致嘉金属科技有限公司91441900590063932M"] = \
                "4413960FRK 惠州市致嘉模具有限公司 91441322MAELYTFD8K"
            replacements["境内发货人:441996482N 东莞致嘉金属科技有限公司91441900590063932M"] = \
                "境内发货人:4413960FRK                                                      惠州市致嘉模具有限公司 91441322MAELYTFD8K"
            # 最后全局替换“东莞”为“惠州”（用于境内货源地）
            replacements["东莞"] = "惠州"
...
def render():
    st.header("东莞/惠州致嘉 · 出口单证自动生成")
    st.caption("上传 Excel 发票，自动生成：申报要素(Word)、成交确认书(Word)、出口报关单(Excel)，打包为一个 ZIP 下载。")

    # ========== 工厂选择 ==========
    factory = st.selectbox("选择工厂", ["东莞致嘉", "惠州致嘉"], key="factory_select")
    ...
    if st.button("生成文档", type="primary", key="zhijia_btn"):
        ...
        with st.spinner("正在解析发票并生成单证，请稍候…"):
            ...
            try:
                ...
                # ========== 惠州致嘉：覆盖公司名 ==========
                if factory == "惠州致嘉":
                    invoice_data["company_name"] = "惠州市致嘉模具有限公司"
